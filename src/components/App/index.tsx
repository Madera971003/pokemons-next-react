...
// Types
import { PokemonData, PokemonDataGenderSpecies } from '../../types/types';
// Styles
...
export function App(): JSX.Element {
  ...
  const callApiByGenderAndColor = (isPrev: boolean): void => {
    if (!isPrev) {
      setPokemosData([]);
  ...
  const handleChangePage = (url: string, isPrev = false): void => {
    if (sortType === 'id') {
      ...
      callApiByGenderAndColor(isPrev);
    }
  };
  ...
  useEffect(() => {
    if (start === 0 && pokemonGenderSpeciesData.length > 0 && flag) {
      setFlag(false);
      callApiByGenderAndColor(false);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [callApiByGenderSpecies, gender]);
